...
export default function StartScreen({ setToken, requestOtp, verifyOtp, onBackToSocial, onEnterArcade, slackCode }) {
  ...
  const code = slackCode || searchParams.get("code");
      ...
      const fetchToken = async () => {
        try {
          ...
          if (res.ok && data?.token) {
            ...
            // Automatically enter the arcade after successful Slack login
            onEnterArcade?.();
          } else {
            console.error(
              "Failed to get token:",
  ...
  const onVerify = async () => {
    ...
    if (result?.ok && result?.token) {
      ...
      // Automatically go directly to Shiba Arcade view after successful verification
      onEnterArcade?.();
    } else {
