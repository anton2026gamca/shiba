...
export default function PlayGameComponent({ gameId, width = "100%", apiBase, style, gameName, thumbnailUrl, animatedBackground, token, onPlayCreated, onGameStart, onGameEnd, gamePageUrl, compact = false, activeGameId = null, isFromMainPage = false }) {
  ...
  // Reset component when this game is no longer active
  useEffect(() => {
    ...
    // Update the wasActive ref
    wasActiveRef.current = isCurrentlyActive;
  }, [activeGameId, gameId, started]);

  // Handle escape key to end game (no logging, just functionality)
  useEffect(() => {
    if (!started) return;
      ...
      {started && (
        <>
          ...
          <div
            style={{
              position: "absolute",
              ...(isFromMainPage ? { bottom: "8px" } : { top: "8px" }),
              right: "8px",
              display: "flex",
              alignItems: "center",
              gap: "8px",
