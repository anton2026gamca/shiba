...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Check if game is not being followed
    if (!currentFollowingGames.includes(gameId)) {
      return res.status(200).json({ 
        ok: true, 
        message: 'Game is not being followed',
        followingGames: currentFollowingGames
      });
    }

    // Remove the game from FollowingGames
    const updatedFollowingGames = currentFollowingGames.filter(id => id !== gameId);
    ...
    return res.status(200).json({ 
      ok: true, 
      message: 'Successfully removed game from FollowingGames',
      followingGames: updatedFollowingGames
    });
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('UnpawProject error:', error);
